...
const Sidebar: React.FC = () => {
	...
	return (
		...
		>
			...
			<div className="no-scrollbar flex flex-col overflow-y-auto pt-4">
				<nav>
					<ul className="mb-6 flex flex-col gap-1.5">
						{menu[keyMenu].map((item) => {
							return (
								<li key={item.to} className={s.wrapper}>
									<NavLink
										to={item.to}
										className={({ isActive }) =>
											'group relative flex items-center gap-2.5 px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-gray-300 ' +
